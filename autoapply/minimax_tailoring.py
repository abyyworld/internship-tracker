...
def minimax_key_path(home: Path) -> Path:
    return home / "minimax.key"


def minimax_key_configured(home: Path) -> bool:
    try:
        return bool(load_minimax_key(home))
    except (FileNotFoundError, RuntimeError):
        return False


def load_minimax_key(home: Path) -> str:
    path = minimax_key_path(home)
    if not path.exists():
        raise FileNotFoundError("MiniMax API key is not configured")
    if path.is_symlink() or path.stat().st_mode & 0o077:
        raise RuntimeError("MiniMax key must be a private regular file with mode 0600")
    value = path.read_text(encoding="utf-8").strip()
    if len(value) < 20 or any(character.isspace() for character in value):
        raise RuntimeError("MiniMax API key is invalid")
    return value


def save_minimax_key(home: Path, value: str) -> None:
    key = str(value).strip()
    if len(key) < 20 or any(character.isspace() for character in key):
        raise ValueError("Paste a valid MiniMax API key")
    home.mkdir(parents=True, exist_ok=True, mode=0o700)
    home.chmod(0o700)
    path = minimax_key_path(home)
    if path.exists() and path.is_symlink():
        raise RuntimeError("Refusing a symbolic-link MiniMax key")
...
def _json_object(value: str) -> dict[str, Any]:
    ...
    if start < 0 or end <= start:
        raise RuntimeError("MiniMax returned no JSON suggestion object")
    try:
        parsed = json.loads(cleaned[start : end + 1])
    except json.JSONDecodeError as exc:
        raise RuntimeError("MiniMax returned malformed suggestion JSON") from exc
    if not isinstance(parsed, dict):
        raise RuntimeError("MiniMax returned an invalid suggestion object")
    return parsed
...
def generate_suggestions(
    ...
    *,
    ...
    try:
        ...
        raise RuntimeError(
            "MiniMax M3 timed out. Retry once; no CV changes were saved."
        ) from exc
    except requests.HTTPError as exc:
        status = exc.response.status_code if exc.response is not None else "unknown"
        raise RuntimeError(
            f"MiniMax API returned HTTP {status}. Check the key and account balance."
        ) from exc
    except (requests.RequestException, KeyError, IndexError, TypeError, ValueError) as exc:
        raise RuntimeError(f"MiniMax M3 request failed: {exc}") from exc
    ...
    if not draft["bullets"] and not draft["summary"]:
        ...
        raise RuntimeError(
            "MiniMax produced no evidence-safe suggestions. "
