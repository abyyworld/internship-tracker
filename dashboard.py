...
<section class="helper">
  <div><strong>⚡ AI CV Studio</strong><p>Every role opens your complete CV with reviewable AI suggestions. Accept, reject, directly edit, then export — nothing is silently removed.</p>
  <!-- Two ways in, and the buttons never lead anywhere dead. "Edit CV for this
       job" prefers the helper on the reader's own machine, which is the better
       editor, and falls back to studio.html in the browser when there is no
       helper — so someone who has installed nothing still gets their CV
       tailored. -->
  <details class="helper-help"><summary>Do I have to install anything?</summary>
  <p>No. <a href="./studio.html">CV Studio in the browser</a> tailors your CV here — your CV, your key and your edits stay in this browser, and the rewrite goes straight from this page to the model provider you choose.</p>
  <p>The version that runs on your own machine is better where it counts: it reads the advert for you, checks every claim against your fact bank, keeps a library of tailored CVs, and exports a real PDF. Double-click <code>install-login-service.command</code> in the project folder once and it starts at every login, keeps itself up to date, and needs no window kept open.</p></details></div>
  <a class="btn primary" href="./studio.html" title="Tailor your CV here, with nothing installed">Tailor a CV now</a>
  <a class="btn secondary" href="./open.html" target="_blank" title="Checks the CV helper on this machine, pairs this browser with it, and reports which build, provider and model it is set up to use">Connect this browser</a>
</section>
...
<div class="fitbar" id="fitOffer">
  ...
  <span class="spacer"></span>
  <a class="btn primary" href="./open.html?to=dashboard">Open my local tracker</a>
</div>
...
function card(j){
  ...
  // Not a direct link to 127.0.0.1: when the helper is not running that answers
  // with a browser error page that explains nothing. open.html tries the helper
  // and, when nothing is listening, hands over to studio.html, which does the
  // same job in the browser with nothing installed. Everything known about the
  // posting travels along, because neither destination can look it up: the
  // helper may never have imported it, and a web page cannot read another
  // site's advert at all.
  const localUrl=`./open.html?url=${encodeURIComponent(j.url)}`+
    `&company=${encodeURIComponent(j.company||"")}&role=${encodeURIComponent(j.role||"")}`+
    (j.location?`&location=${encodeURIComponent(j.location)}`:"")+
    (j.focus?`&tags=${encodeURIComponent(j.focus)}`:"");
  const f=fitOf(j);
